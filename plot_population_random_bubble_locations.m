...
if ~exist('plot_population_random_bubble_locations.mat','file')
    %% plot signals
    Seeds = [1122,1222,1322,1422,1522,1622,1722,1822,1922,2022];
    caselab = {};
    SHpsd = zeros(6,length(Seeds));
    SHdem = zeros(6,length(Seeds));
    for seed_ndx = 1:length(Seeds)
        seednum = Seeds(seed_ndx);
        fname = ['population_003_L',num2str(seednum)];
        fname = [fname,'_Povs.mat'];
        caselab{seed_ndx} = ['L-seed = ',num2str(seednum)];
        load(fname); 
    ...
    save('plot_population_random_bubble_locations.mat','*');
else
    load('plot_population_random_bubble_locations.mat');
end
