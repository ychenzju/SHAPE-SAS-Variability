...
histogram(Rs*1e6,12);
subplot(1,2,2);
scatter3(Xs,Ys,Zs,Rs*1e6,'r'); xlabel('X'); ylabel('Y'); zlabel('Z'); title('Randomly generated bubbles');
...
title('Scattered Signals from Each Bubble')
xlabel('Time (\mus)');
ax.YTickLabel = [];
...
legend('Transmitted acoustic signal','Driving pulse')
subplot(3,2,4);
X = Treceived{pov_ndx} * 1e6;
Y = Preceived{pov_ndx};
...
xlabel('Time (\mus)');
ylabel('Pressure (Pa)');
title('Received acoustic signal');
xlim([X(1),X(end)]);
...
for pov_ndx = 1:length(povs)
    for frq_ndx = 1:length(frqs)
        for pac_ndx = 1:length(pacs)
            ...
            subplot(121), plot(Tt*1e6,Pt); grid on; hold on; YL = ylim();
...
xlabel('Ambient Pressure (kPa)');
ylabel('Amplitude (dB)');
title('Harmonic Response (PSD) vs. Ambient Pressure')
