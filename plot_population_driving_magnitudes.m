...
sh = reshape(xsh, length(povs), length(pacs));
sh_pi = reshape(xsh_pi, length(povs), length(pacs));
sh_am = reshape(xsh_am, length(povs), length(pacs));
sh_cps = reshape(xsh_cps, length(povs), length(pacs));
...
pacs_selected = [100,200,300,400];
[~,ia,ib] = intersect(pacs_selected,pacs/1e3);
pacs_selectedIdx = ib;

povs_labels = compose("Pov = %2.0f kPa",povs(povs_selectedIdx)/1e3);
pacs_labels = compose("Pac = %2.0f kPa",pacs(pacs_selectedIdx)/1e3);

fig = figure(5006); fig.Position = [200,40,1000,1080];
subplot(4,2,1);
X = pacs(:) / 1e3;
...
subplot(4,2,2);
X = povs(:) / 1e3;
Y = sh(:,pacs_selectedIdx) - sh(1,pacs_selectedIdx);
plt = plot(X,Y); grid on;
for k=1:length(plt)
...
subplot(4,2,4);
X = povs(:) / 1e3;
Y = sh_pi(:,pacs_selectedIdx) - sh_pi(1,pacs_selectedIdx);
plt = plot(X,Y); grid on;
for k=1:length(plt)
    plt(k).Color=colors{k}; plt(k).LineStyle=styles{k}; plt(k).Marker=mrkers{k}; plt(k).LineWidth=widths{k}; plt(k).MarkerSize=4;
end
...
subplot(4,2,6);
X = povs(:) / 1e3;
Y = sh_am(:,pacs_selectedIdx) - sh_am(1,pacs_selectedIdx);
plt = plot(X,Y); grid on;
for k=1:length(plt)
    plt(k).Color=colors{k}; plt(k).LineStyle=styles{k}; plt(k).Marker=mrkers{k}; plt(k).LineWidth=widths{k}; plt(k).MarkerSize=4;
end
...
subplot(4,2,8);
X = povs(:) / 1e3;
Y = sh_cps(:,pacs_selectedIdx) - sh_cps(1,pacs_selectedIdx);
plt = plot(X,Y); grid on;
for k=1:length(plt)
    plt(k).Color=colors{k}; plt(k).LineStyle=styles{k}; plt(k).Marker=mrkers{k}; plt(k).LineWidth=widths{k}; plt(k).MarkerSize=4;
end
