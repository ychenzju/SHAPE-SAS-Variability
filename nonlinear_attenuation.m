function [output_signal,impulse_response,h,f] = nonlinear_attenuation(alpha,gamma,depth,Ts,input_signal)

%% input arguments
Ts = Ts * 1e6; %microseconds
NFFT = 11; NPT = 2^NFFT; N2 = NPT / 2;
%% generate attenuation frequency responses
[impulse_response,h,f] = frqrsp(alpha,gamma,depth,Ts,NFFT);
...
function [impulse_response,h,f] = frqrsp(alpha,gamma,depth,Ts,NFFT)
...
for ndx=2:N2P1
    ...
    xmag(ndx) = -alpha * (frqMhz)^gamma * depth / 20;
