...
caselab{1} = ['Bubbles without Interaction'];
caselab{2} = ['Bubbles with Interaction'];    
for case_ndx = 1:2
    switch case_ndx
        case 1
    ...
    for pov_ndx = 1 : length(povs)
        ...
        subplot(length(povs),3,(pov_ndx-1)*3+1);
        X = Treceived{pov_ndx} * 1e6;
        Y = Preceived{pov_ndx};
    ...
    xlabel('Time (\mus)');
    ylabel('Radius (\mum)');
    title('Radius Dynamics Comparison');
    ...
    xlabel('Time (\mus)');
    ylabel('Radius (\mum)');
    title('Radius Dynamics of Bubble #16');
    xlim([X(1),X(end)]);
...
subplot(2,2,1);
legend('With Interaction','Without Interaction');
subplot(2,2,2);
legend('With Interaction','Without Interaction');
...
xlabel('Ambient Pressure (kPa)');
ylabel('Amplitude (dB)');
title({'Subharmonic Response from Population','IQ Demodulation Method'});
subplot(2,2,4);
X = povs / 1e3;
...
xlabel('Ambient Pressure (kPa)');
ylabel('Amplitude (dB)');
title({'Relative Subharmonic Response from Population','IQ Demodulation Method'});
